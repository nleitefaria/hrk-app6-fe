...
import com.mycompany.hrkapp6fe.dto.EmployeesPageDTO;

public class EmployeeService {
	
	final String path = "https://hrk-app6.herokuapp.com/api/v1/employee"; 
	
	public EmployeeService(){}
	
    public List<EmployeesDTO> getAll(int start, int size)
    {  
    	ResteasyClient client = new ResteasyClientBuilder().build();
        ResteasyWebTarget target = client.target(path + "/all/" + (start + 1));
        Response response = target.request().get();
        EmployeesPageDTO value = response.readEntity(EmployeesPageDTO.class);
        response.close();
