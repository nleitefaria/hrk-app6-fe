...
import com.mycompany.hrkapp6fe.dto.OrdersPageDTO;

public class OrderService {
	
	final String path = "https://hrk-app6.herokuapp.com/api/v1/order"; 
	
	public OrderService(){}
	
    public List<OrdersDTO> getAll(int start, int size)
    {  
    	ResteasyClient client = new ResteasyClientBuilder().build();
        ResteasyWebTarget target = client.target(path + "/all/" + (start + 1));
        Response response = target.request().get();
        OrdersPageDTO value = response.readEntity(OrdersPageDTO.class);
        response.close();
